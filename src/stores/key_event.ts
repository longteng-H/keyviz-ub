...
const createKeyEventStore = createSyncedStore<KeyEventStore>(
    ...
    (set, get) => ({
                ...
                // history mode, last group has only this key
                    ...
                    groups[last].keys.forEach(gKey => {
                        if (gKey.name === key.name) {
                            // update existing key's pressed count and time (create new instance for Zustand reactivity)
                            const newKey = new KeyEvent(gKey.name);
                            newKey.pressedCount = gKey.pressedCount + 1;
                            groupKeys.push(newKey);
                        } else if (gKey.in(pressedKeys)) {
                            groupKeys.push(gKey);
                        }
                    });
                    groups[last] = { ...groups[last], keys: groupKeys };
                }
            }
            // 3. add to group
            else {
                ...
                if (pressedKeys.length === 1 || last < 0) {
                    if (state.showEventHistory) {
                        ...
                        // replace mode: add to existing group or create new one
                        if (last >= 0) {
        ...
        onKeyRelease(event: RawKeyEvent) {
            ...
            if (kIndex !== undefined && kIndex !== -1) {
                const updatedKeys = [...groups[last].keys];
                const oldKey = updatedKeys[kIndex];
                updatedKeys[kIndex] = new KeyEvent(oldKey.name);
                updatedKeys[kIndex].pressedCount = oldKey.pressedCount;
                groups[last] = { ...groups[last], keys: updatedKeys };
                set({ pressedKeys, groups });
            } else {
                set({ pressedKeys });
            }
        ...
        onMouseMove(event: MouseMoveEvent) {
            ...
            if (mouse.dragStart && !mouse.dragging) {
                ...
                if (dragDistance > state.dragThreshold) {
                    ...
                    const last = groups.length - 1;
                    if (last >= 0) {
                        groups[last] = { ...groups[last], keys: groups[last].keys.filter(key => key.name !== state.pressedMouseButton?.toString()) };
                    }
        ...
        tick() {
            ...
            if (state.mouse.lastScrollAt && now - state.mouse.lastScrollAt > SCROLL_LINGER_MS) {
                const latestKeys = get().pressedKeys;
                if (latestKeys.includes(RawKey.ScrollUp)) {
                    state.onKeyRelease({ type: "KeyEvent", name: RawKey.ScrollUp, pressed: false });
                }
                if (latestKeys.includes(RawKey.ScrollDown)) {
            ...
            for (const group of state.groups) {
                const updatedKeys = group.keys.filter((key) => {
                    // Re-read pressedKeys on each key to avoid stale snapshot race with onKeyRelease
                    const latestKeys = get().pressedKeys;
                    return (
                        latestKeys.includes(key.name) ||
