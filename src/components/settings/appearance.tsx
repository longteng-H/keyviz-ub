...
export const AppearanceSettings = () => {
    ...
    return <div className="flex flex-col gap-y-4 p-6">
        ...
        <h2 className="text-sm text-muted-foreground font-medium">{t('appearance.position.title')}</h2>
        {
            monitors.length > 1 &&
            <Item variant="muted">
                <ItemContent>
                    <ItemTitle>
                        ...
                        {t('appearance.position.display.title')}
                    </ItemTitle>
                    <ItemDescription>
                        {t('appearance.position.display.description')}
                    </ItemDescription>
                </ItemContent>
                <ItemActions>
                    <Select
                    ...
                    >
                        <SelectTrigger className="w-32">
                            <SelectValue placeholder={t('appearance.position.display.placeholder')} />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectGroup>
                                {
                                    monitors.map((monitor, index) => (
                                        <SelectItem key={monitor.name} value={monitor.name ?? index.toString()}>
                                            {monitor.name ?? `${t('appearance.position.display.placeholder')} ${index + 1}`} ({monitor.size.width}x{monitor.size.height})
                                        </SelectItem>
                                    ))
                                }
                            </SelectGroup>
        ...
        <Item variant="muted">
            <ItemContent className="self-start">
                <ItemTitle>
                    <HugeiconsIcon icon={TextAlignLeftIcon} size="1em" /> {t('appearance.position.alignment.title')}
                </ItemTitle>
                <ItemDescription>
                    {t('appearance.position.alignment.description')}
                </ItemDescription>
            </ItemContent>
            <ItemActions>
                <AlignmentSelector
        ...
        <Item variant="muted">
            <ItemContent>
                <ItemTitle>
                    <HugeiconsIcon icon={ParagraphSpacingIcon} size="1em" /> {t('appearance.position.margin.title')}
                </ItemTitle>
                <ItemDescription>
                    {t('appearance.position.margin.description')}
                </ItemDescription>
            </ItemContent>
            <ItemActions>
                <NumberScrubber
        ...
        <h2 className="text-sm text-muted-foreground font-medium">{t('appearance.animation.title')}</h2>
        <Item variant="muted">
            <ItemContent>
                <ItemTitle>
                    <HugeiconsIcon icon={Time03Icon} size="1em" /> {t('appearance.animation.duration.title')}
                </ItemTitle>
                <ItemDescription className="max-w-84">
                    {t('appearance.animation.duration.description')}
                </ItemDescription>
            </ItemContent>
            <ItemActions>
                <NumberInput
        ...
        <Item variant="muted">
            <ItemContent>
                <ItemTitle>
                    <HugeiconsIcon icon={KeyframesDoubleIcon} size="1em" /> {t('appearance.animation.type.title')}
                </ItemTitle>
            </ItemContent>
            <ItemActions>
                <Select value={appearance.animation} onValueChange={(value) => setAppearance({ animation: value as any })}>
                    ...
                    <SelectContent>
                        <SelectGroup>
                            <SelectItem value="none">{t('appearance.animation.type.options.none')}</SelectItem>
                            <SelectItem value="fade">{t('appearance.animation.type.options.fade')}</SelectItem>
                            <SelectItem value="zoom">{t('appearance.animation.type.options.zoom')}</SelectItem>
                            <SelectItem value="float">{t('appearance.animation.type.options.float')}</SelectItem>
                            <SelectItem value="slide">{t('appearance.animation.type.options.slide')}</SelectItem>
                        </SelectGroup>
                    </SelectContent>
                </Select>
            </ItemActions>
        ...
        <Item variant="muted">
            <ItemContent>
                <ItemTitle>
                    <HugeiconsIcon icon={KeyframesDoubleRemoveIcon} size="1em" /> {t('appearance.animation.speed.title')}
                </ItemTitle>
                <ItemDescription>
                    {t('appearance.animation.speed.description')}
                </ItemDescription>
            </ItemContent>
            <ItemActions>
                <NumberInput
